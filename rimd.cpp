...
RIMD::RIMD(Mesh& base, Mesh& target){
    this->baseMesh = &base;
    this->targetMesh = &target;
    solveTransform();
    this->features = solveRIMD();
}

RIMD::RIMD(Mesh& base){
    this->baseMesh = &base;
    this->targetMesh = &base;

}

Eigen::MatrixXd RIMD::solveReconstruct(const std::vector<std::vector<Eigen::Matrix3d> >& rimd_vectors){
...
Eigen::MatrixXd RIMD::quickReconstruct(const std::vector<std::vector<Eigen::Matrix3d>>& rimd_vectors) {
    // Ti * e_ij = e'_ij
...
std::vector<Eigen::Matrix3d> RIMD::polarDecomposition(const Eigen::Matrix3d& T){
...
std::vector<std::vector<Eigen::Matrix3d> > RIMD::solveRIMD(){
    // f = {log_dR_ij; Si}
...
Eigen::Matrix3d RIMD::compute_ring(const std::vector<std::vector<Eigen::Matrix3d> >& rimd_vectors, const std::vector<Eigen::Matrix3d>& R, int j) {
...
void RIMD::BFS_initialize(const std::vector<std::vector<Eigen::Matrix3d>>& rimd_vectors, std::vector<Eigen::Matrix3d>& R) {
...
void RIMD::solveGlobalStep(Eigen::SimplicialCholesky<Eigen::SparseMatrix<double> >& solver, Eigen::VectorXd& x, Eigen::VectorXd& b, const std::vector<Eigen::Matrix3d>& R, const std::vector<std::vector<Eigen::Matrix3d> >& rimd_vectors){
...
void RIMD::solveLocalStep(const Eigen::VectorXd& x, std::vector<Eigen::Matrix3d>& R, const std::vector<std::vector<Eigen::Matrix3d> >& rimd_vectors){
...
double RIMD::computeEnergy(const Eigen::VectorXd& x, const std::vector<Eigen::Matrix3d>& R, const std::vector<std::vector<Eigen::Matrix3d> >& rimd_vectors){
    ...
    return energy / num_verts;
}

void RIMD::serialize(const std::string filename){
    std::string line, word;
    std::ofstream file;
    file.open(filename);
    for (int i=0; i<this->baseMesh->verts.rows(); i++){
        for (int j=0;j<this->baseMesh->neighbors[i].size()+1; j++){
            for (int k=0; k<3; k++){
                for (int h=0; h<3; h++){
                    file<<" "<<this->features[i][j](h,k);
                }
            }
        }
    }
...
void RIMD::deserialize(const std::string filename){
    std::string line, word;
    std::ifstream file;
    int flag=0;
    std::vector<std::vector<Eigen::Matrix3d >> rimd_features;
    ...
    while (getline(file, line)){
        std::stringstream ss1(line);
            rimd_features.resize(baseMesh->verts.rows());
            for(int i=0; i<baseMesh->verts.rows();++i){
                rimd_features[i].resize(baseMesh->neighbors[i].size()+1);
            }
            int a=0;
            int b=0;
            int c=0;
            ...
            while(ss1.rdbuf()->in_avail() != 0){
                ss1>>temp;
                    ...
                    if (b==this->baseMesh->neighbors[a].size()+1){
                        b=0;
                        ++a;
                    }
                    if(a==baseMesh->verts.rows()){
                        flag=2;
                    }
